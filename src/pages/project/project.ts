...
export class ProjectPage {public person: User = {
  userId: '',
  name: '',
  phone: '',
  pic: '',
...
getStories() {
  ...
  loading.present();
  console.log(this.person);
  this.restProvider.getData('content/user/'+this.person.userId+'/project')
    .then(data => {
      this.project = data;
...
doRefresh(refresher) {
  ...
  public Edit(id:any) {
  console.log(id);
  let createModal = this.modalCtrl.create("EditprojectPage",{iddata: id});
  createModal.present();
